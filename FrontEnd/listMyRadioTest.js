        ...
        if(this.readyState=4 && this.status==200)
        {
            ...
            for(var i=0;i<data.length;i+=1)
            {
                if(data[i].empId!=empId)
                    continue;
                innerCode+="<tr><td class=\"other\">"+data[i].examRadioId+"</td><td class=\"other\">"+data[i].examRadioName+"</td><td class=\"other\">"+data[i].examRadioDetails+"</td><td class=\"other\">"+data[i].examStatus+"</td><td class=\"other\">";
                if(data[i].examRadioRes==0)
                    innerCode+="Not Uploaded";
                else
                    innerCode+="<input type=\"button\" value=\"View Results\" onclick=\"download("+data[i].examRadioRes+")\" style=\"border-radius: 20px\">";
                innerCode+="</td></tr>";
